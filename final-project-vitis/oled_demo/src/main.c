...
PmodOLED myDevice;
#define OLED_X_DIM 128
#define OLED_Y_DIM 32
...
void DrawDividers() {
	...
	OLED_SetCursor(&myDevice, 0, 1);
	OLED_PutString(&myDevice, "REC");
	OLED_SetCursor(&myDevice, 0, 2);
	OLED_PutString(&myDevice, "RDY");
	OLED_SetCursor(&myDevice, 0, 3);
	OLED_PutString(&myDevice, "PLAY");

	OLED_SetCursor(&myDevice, 5, 0);
	for(int i = 0; i < 4; i++) {
		OLED_PutChar(&myDevice, ' ');
		OLED_PutChar(&myDevice, i + '0');
	}
...
void MarkCell(u8 row, u8 col) {
	...
	OLED_SetCursor(&myDevice, col*2 + 6, row + 1);
	OLED_PutChar(&myDevice, 'x');
	OLED_Update(&myDevice);
}
